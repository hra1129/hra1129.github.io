...
typedef enum {
	...
	CZMA_REPEAT, CZMA_ENDR, CZMA_ORG, CZMA_GLOBAL_SYMBOL, CZMA_DEFB, CZMA_DEFW, CZMA_DEFD, CZMA_DEFS,
	...
	CZMA_SLA, CZMA_SRA, CZMA_SRL, CZMA_ADD, CZMA_ADC, CZMA_INC, CZMA_SUB, CZMA_SBC,
...
bool CZMA_PARSE_BIT::process( CZMA_INFORMATION& info, CZMA_PARSE* p_last_line ) {
	...
	update_flags( &info, p_last_line );
	if( this->opecode_n_sss( info, 0x40 ) ) {
		//	log
		if( !this->is_analyze_phase ) {
