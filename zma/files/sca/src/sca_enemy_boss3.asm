...
enemy_boss3_move::
		...
		or		a, a
		jr		nz, enemy_boss3_move_active
enemy_boss3_inactive:
		...
		ret
enemy_boss3_move_active:
		ld		a, [ix + SCA_INFO_YH]
		add		a, [ix + SCA_INFO_ENEMY_STATE_L]	; Y方向の増分
		cp		a, 192-32
		jp		c, enemy_boss3_move_y
		cp		a, 240
		jp		c, enemy_boss3_move_y_right
		ld		a, 3
		ld		[ix + SCA_INFO_ENEMY_STATE_L], a
		xor		a, a
		jp		enemy_boss3_move_y
enemy_boss3_move_y_right:
		ld		a, -3
		ld		[ix + SCA_INFO_ENEMY_STATE_L], a
		ld		a, 192-32
enemy_boss3_move_y:
		...
		ld		a, [ix + SCA_INFO_XH]
		add		a, [ix + SCA_INFO_ENEMY_STATE_H]	; X方向の増分
		cp		a, 192-32
		jp		c, enemy_boss3_move_x
		cp		a, 240
		jp		c, enemy_boss3_move_x_bottom
		ld		a, 3
		ld		[ix + SCA_INFO_ENEMY_STATE_H], a
		xor		a, a
		jp		enemy_boss3_move_y
enemy_boss3_move_x_bottom:
		ld		a, -3
		ld		[ix + SCA_INFO_ENEMY_STATE_H], a
		ld		a, 192-32
enemy_boss3_move_x:
...
enemy_boss3_move_fire:
		; 弾を発射するタイミングか？
		ld		a, [ix + SCA_INFO_ENEMY_SHOT_TIMING]
		inc		a
		and		a, 7
		ld		[ix + SCA_INFO_ENEMY_SHOT_TIMING], a
		jr		nz, enemy_boss3_move_end					; 発射のタイミングでなければ enemy_boss3_move_end へ
		; 発射処理
		ld		iy, player_info
		call	enemy_shot_start
enemy_boss3_move_end:
enemy_boss3_move_dummy:
		ret
